...
class PoolsyncConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_poolsync_login(
        self, step_id, user_input: dict[str, Any] | None, schema: vol.Schema
    ) -> FlowResult:
        """Attempt a login with Poolsync."""
        errors = {}

        poolsync = Poolsync(username=user_input[CONF_USERNAME])
        try:
            await self.hass.async_add_executor_job(
                poolsync.authenticate, user_input[CONF_PASSWORD]
            )
        except PoolsyncAuthenticationError:
        ...
        if not errors:
            return await self._async_create_entry(user_input | poolsync.get_tokens())
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            return await self.async_poolsync_login(
